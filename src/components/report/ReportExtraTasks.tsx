...
export const ReportExtraTasks = ({ tasks }: ReportExtraTasksProps) => {
  ...
  return (
    <div
      className="animate-fade-in-up space-y-6 rounded-[10px] bg-[#EAF1FF] px-6 py-8"
      style={{ animationDelay: "0.4s" }}
    >
      <div className="flex items-baseline justify-between">
        <h2 className="text-lg font-bold text-foreground">Prace dodatkowe - w Pakiecie</h2>
      ...
      <ul className="space-y-4">
        {tasks.map((task, index) => (
          <li key={index} className="flex items-start gap-3">
            <CircleCheck className="mt-0.5 h-5 w-5 shrink-0 text-[#5B7EE5]" />
